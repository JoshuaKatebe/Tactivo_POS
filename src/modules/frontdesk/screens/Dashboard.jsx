...
export default function FrontdeskDashboard() {
  const { pumps: pumpsData, tanks: tanksData, loading } = useFuel();

  // Convert pumps object to array and sort by Pump ID
  const pumps = useMemo(() => {
    return Object.values(pumpsData).sort((a, b) => a.Pump - b.Pump);
  }, [pumpsData]);

  // Convert tanks object to array and sort by Tank ID
  const tanks = useMemo(() => {
    return Object.values(tanksData).sort((a, b) => a.Tank - b.Tank);
  }, [tanksData]);

  // Calculate generic percentages for the dashboard summary if needed, or map real data
  const tankLevels = useMemo(() => {
    return tanks.map(t => ({
      name: t.FuelGradeName || t.ProductName || `Tank ${t.Tank}`,
      pct: t.TankFillingPercentage || 0,
      volume: t.ProductVolume,
      capacity: t.Capacity || (t.ProductVolume / (t.TankFillingPercentage / 100 || 1)) // Estimate if missing
    }));
  }, [tanks]);
