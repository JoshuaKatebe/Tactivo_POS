...
import QuickClearPanel from "./QuickClearPanel";
...
export default function ProductsPanel({
  ...
  // If pump category is selected, show the full PumpsPage
  if (currentCategory === "pump") {
    return (
      <div className="w-1/2 min-w-[400px] p-4 bg-gradient-to-b from-blue-50 to-blue-100 
                      border-r border-blue-300 text-black flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto">
          <PumpsPage
            initialPumps={pumps.length > 0 ? pumps : null}
            onClearTransaction={onClearAttendant}
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="w-1/2 min-w-[400px] p-4 bg-gradient-to-b from-blue-50 to-blue-100 
      ...
      {/* Quick Clear Panel - Real-time pending transactions */}
      <QuickClearPanel
        stationId={stationId}
                  ...
                  return (
                    <div key={tx.id} className={`p-3 rounded-lg border ${isPending ? "bg-amber-50" : "bg-gray-50"}`}>
                      <div className="flex justify-between">
                        <div>
                          <div className="font-medium">{tx.sku || tx.id}</div>
